...
**BigQuery is no longer in this list** - it's a real module now
([`_12_bigquery`](../src/main/java/com/ashfaq/gcplab/_12_bigquery)), the
analytics backbone the KPI tracking in `_09`'s Production practices section
assumes exists (search-query logs, click/conversion history, A/B test
results - exactly the "which queries have the worst zero-results rate"
kind of question BigQuery answers and Cloud Monitoring dashboards don't).
